...
WasmExecutor::Execute(const InvocationRequest& request, IAuditSink& audit) const {
    ...
    if (payload_ptr + payload_json.size() > memory_size) {
        wasmtime_linker_delete(linker);
        wasmtime_module_delete(module);
        wasmtime_store_delete(store);
        audit.LogExecutionFailure(request, ExecutionError::UnsupportedFunction);
    ...
    if (result_ptr + result_len <= memory_size) {
        result.success = true;
        ...
        result.error = "Memory out of bounds";
    }

    // Step 5: Free Memory
    wasmtime_val_t free_args[2] = {
        {.kind = WASMTIME_I32, .of = {.i32 = static_cast<int32_t>(result_ptr)}},
        {.kind = WASMTIME_I32, .of = {.i32 = static_cast<int32_t>(result_len)}}
    };
    error = wasmtime_func_call(context, &ext_free.of.func, free_args, 2, nullptr, 0, &trap);
    if (error) wasmtime_error_delete(error);
    if (trap) wasm_trap_delete(trap);
