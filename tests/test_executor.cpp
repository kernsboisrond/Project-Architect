...
int main() {
    ...
    check_error(executor.Execute(req1), ExecutionError::UnsupportedModule, "Unsupported Module");

    std::cout << "\n--- Test 2: Unsupported Function ---\n";
    InvocationRequest req2;
    req2.module_name = "echo";
    req2.function_name = "format";
    check_error(executor.Execute(req2), ExecutionError::UnsupportedFunction, "Unsupported Function");

    std::cout << "\n--- Test 3: Invalid Arguments (Missing) ---\n";
    InvocationRequest req3;
    req3.module_name = "echo";
    req3.function_name = "print";
    check_error(executor.Execute(req3), ExecutionError::InvalidArguments, "Invalid Arguments");
    ...
    auto res4 = executor.Execute(req4);
